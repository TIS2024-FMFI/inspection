...
$username_db = getenv('DB_USER') ?: 'safer_app_user';
...
<body>
    ...
    ");
                ...
                ");
                    ...
                    $stmt = $pdo->prepare("
                UPDATE product_history ph
                JOIN user_submitted_products pl ON ph.barcode = pl.barcode
                SET ph.product_link = 'personalized_list.php'
                WHERE ph.user_id = :user_id
                ");
                    $stmt->execute(['user_id' => $user_id]);
                    ...
                    if (count($historyItems) > 0) {
                        foreach ($historyItems as $row) {
                            ...
                            echo 'Barcode: ' . htmlspecialchars($row['barcode']) . ' | ';
